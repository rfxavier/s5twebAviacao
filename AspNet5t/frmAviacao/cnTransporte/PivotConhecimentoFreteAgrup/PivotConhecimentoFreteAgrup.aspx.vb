...
Public Class PivotConhecimentoFreteAgrup
    ...
    Private Sub ExportRaw(ByVal saveAs As Boolean)
        ...
        options = New DevExpress.XtraPrinting.XlsxExportOptionsEx() With {.ExportType = ExportType.WYSIWYG, .AllowGrouping = DefaultBoolean.False, .RawDataMode = True}
    ...
    Protected Sub ASPxPivotGrid1_CustomCellValue(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxPivotGrid.PivotCellValueEventArgs)
        ...
        '    fieldpVLRTOTALPRODUTO1.Visible = True
        '    fieldpQUANTIDADE1.Visible = True
        ...
        '    fieldpCUSTOPRODUTO1.Visible = True
        '    fieldpQUANTIDADE1.Visible = True
    ...
    Private Function GetCustoProdQtd(ByVal e As DevExpress.Web.ASPxPivotGrid.PivotCellValueEventArgs) As Double
        ...
        Return values
    End Function
    Protected Sub pivotGrid_CustomFieldValueCells(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxPivotGrid.PivotCustomFieldValueCellsEventArgs)

        If ASPxPivotGrid1.DataSource Is Nothing Then
            Return
        End If
        ...
        For i As Integer = e.GetCellCount(True) - 1 To 0 Step -1
            Dim cell As DevExpress.Web.ASPxPivotGrid.FieldValueCell = e.GetCell(True, i)
            If cell Is Nothing Then
                Continue For
            End If
            If cbOcultarCampos.Checked = True Then
                If cell.DisplayText = "Quantidade" Or cell.DisplayText = "Valor Total Produto" Or cell.DisplayText = "Custo Produto" Then
                    e.Remove(cell)
                End If
            End If
        Next i
        ...
        For i As Integer = e.GetCellCount(False) - 1 To 0 Step -1
            Dim cell As DevExpress.Web.ASPxPivotGrid.FieldValueCell = e.GetCell(False, i)
            If cell Is Nothing Then
                Continue For
            End If
            If cbOcultarCampos.Checked = True Then
    ...
    Protected Sub ASPxPivotGrid1_FieldAreaChanging(sender As Object, e As DevExpress.Web.ASPxPivotGrid.PivotAreaChangingEventArgs) Handles ASPxPivotGrid1.FieldAreaChanging
        'If e.Field Is calcPRECOMEDIOVENDA Then

        '    fieldpVLRTOTALPRODUTO1.Visible = True
        '    fieldpQUANTIDADE1.Visible = True

        'ElseIf e.Field Is calcCUSTOPRODQTD Then
    ...
    Private Sub LinqServerModeDataSource1_Selecting(sender As Object, e As LinqServerModeDataSourceSelectEventArgs) Handles LinqServerModeDataSource1.Selecting
        'Dim db As New AviacaoConhecimentoFreteBasedataclassDataContext
        'e.KeyExpression = "pId"
        'e.QueryableSource = From aviacaoconhecimentofrete In db.AviacaoConhecimentoFretes Where aviacaoconhecimentofrete.pGW1_NRDC = "000392666"
        ''e.QueryableSource = From aviacaoconhecimentofrete In db.AviacaoConhecimentoFretes Where aviacaoconhecimentofrete.pGW3_DTEMIS >= New Date(2016, 8, 1)
    End Sub

    'Private Sub ASPxPivotGrid1_CustomCellDisplayText(sender As Object, e As DevExpress.Web.ASPxPivotGrid.PivotCellDisplayTextEventArgs) Handles ASPxPivotGrid1.CustomCellDisplayText
    '    If e.DataField IsNot fieldpGW8_VALOR_BRUTO_RATEIO Then Return

    '    e.DisplayText = String.Format(e.Value, "{0:#,#0.00}")


    'End Sub

    Private Sub ASPxPivotGrid1_FieldValueDisplayText(sender As Object, e As DevExpress.Web.ASPxPivotGrid.PivotFieldDisplayTextEventArgs) Handles ASPxPivotGrid1.FieldValueDisplayText
        'If e.Field Is Nothing Then
        '    Return
        'End If
        'If e.Field.FieldName = "pGW8_VALOR_TOTAL_NOTA" Or e.Field.FieldName = "pGW8_VALOR_BRUTO_TOTAL_NOTA" Or e.Field.FieldName = "pF2_PBRUTO" Or e.Field.FieldName = "pF2_PLIQUI" Then
        '    e.DisplayText = FormatNumber(e.Value, 2) 'String.Format(e.Value, "{0:#,#0.00}")
        'End If
    End Sub

    Private Sub ASPxPivotGrid1_CustomCellStyle(sender As Object, e As PivotCustomCellStyleEventArgs) Handles ASPxPivotGrid1.CustomCellStyle
        'If e.DataField IsNot Nothing Then
        '    e.CellStyle.HorizontalAlign = HorizontalAlign.Left
        'End If
    End Sub
End Class