...
Public Class pivotVendasAnoMenos1
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) AndAlso (Not IsCallback) Then
            ...
            mskDatEmissaoIniPesq.Text = New Date(Date.Now.Year, 1, 1).AddYears(-3)
            mskDatEmissaoFinPesq.Text = New Date(Date.Now.Year, Date.Now.Month, Date.Now.Day)
    ...
    Protected Sub ASPxButtonPesquisar_Click(sender As Object, e As EventArgs) Handles ASPxButtonPesquisar.Click
        'ASPxPivotGrid1.Prefilter.CriteriaString = "([fieldpNOMECLIENTE] = 'GIGA BR DISTRIBUIDOR E ATACADISTA LTDA  ' Or [fieldpNOMECLIENTE] = 'GIGA BR DISTRIBUIDOR E ATACADISTA S/A.  ') And [fieldpANOEMISSAO1] = '2016'"
        ...
        'Dim str1 = pivot4tCriteriaString(ASPxPivotGrid1.Prefilter.CriteriaString, ASPxComboBox1.Value.ToString)
        ...
        'If ASPxComboBox1.Value IsNot Nothing Then
        '    If ASPxPivotGrid1.Prefilter.CriteriaString <> "" Then
        '        ASPxPivotGrid1.Prefilter.CriteriaString &= String.Format(" And [fieldpNOMECLIENTE] = '{0}'", ASPxComboBox1.Value.ToString)
        '    Else
        '        ASPxPivotGrid1.Prefilter.CriteriaString = String.Format("[fieldpNOMECLIENTE] = '{0}'", ASPxComboBox1.Value.ToString)
        '    End If
        'End If

        ASPxComboBox1.SelectedIndex = -1
        'ASPxPivotGrid1.Prefilter.CriteriaString = "[fieldpNOMECLIENTE] = '2M GESTAO E PARTICIPACOES SOCIETARIAS LT' And [fieldpANOEMISSAO1] = '2015'"
    End Sub

    'Public Function pivot4tCriteriaString(ByVal parCriteriaString As String, ByVal parCliente As String) As String
    '    pivot4tCriteriaString = parCriteriaString

    '    'ASPxPivotGrid1.Prefilter.CriteriaString.IndexOf("([fieldpNOMECLIENTE] =")

    '    If parCliente IsNot Nothing Then
    '        If parCriteriaString.IndexOf("[fieldpNOMECLIENTE] =") > -1 Then
    '            'JÁ TEM CLIENTE NO CriteriaString PRÉ EXISTENTE
    '            Dim varOcorrenciaAbreCliente = parCriteriaString.IndexOf("[fieldpNOMECLIENTE] =")

    '            'Dim varOcorrenciaFechaCliente = parCriteriaString.IndexOf("')", varOcorrenciaAbreCliente + 1)
    '            Dim varOcorrenciaFechaCliente = parCriteriaString.IndexOfNth("'", varOcorrenciaAbreCliente + 1, 2)

    '            Dim varRestoCriteriaString = parCriteriaString.Substring(varOcorrenciaFechaCliente)

    '            Dim varCriteriaStringParteClienteNovo = String.Format("[fieldpNOMECLIENTE] = '{0}'", parCliente)
    '            Dim varCriteriaStringParteClientesAtuais = parCriteriaString.Substring(varOcorrenciaAbreCliente + 1, varOcorrenciaFechaCliente - varOcorrenciaAbreCliente)

    '            Dim varNovoCriteriaString = String.Format("({0} or {1})", varCriteriaStringParteClientesAtuais, varCriteriaStringParteClienteNovo) & varRestoCriteriaString

    '            parCriteriaString = varNovoCriteriaString
    '        Else
    '            'NÃO TEM CLIENTE NO CriteriaString PRÉ EXISTENTE
    '            If parCriteriaString <> "" Then
    '                parCriteriaString &= String.Format(" And ([fieldpNOMECLIENTE] = '{0}')", parCliente)
    '            Else
    '                parCriteriaString = String.Format("([fieldpNOMECLIENTE] = '{0}')", parCliente)
    '            End If
    '        End If
    '    End If
    ...
    Private Sub LinqServerModeDataSource1_Selecting(sender As Object, e As LinqServerModeDataSourceSelectEventArgs) Handles LinqServerModeDataSource1.Selecting
        ...
        If mskDatEmissaoIniPesq.Text <> "" And mskDatEmissaoFinPesq.Text <> "" Then

            Dim db As New AviacaoVendasAnoMenos1BasedataclassDataContext
            e.KeyExpression = "pId"
            e.QueryableSource = From aviacaoVendas In db.AviacaoVendasAnoMenos1s Where aviacaoVendas.pDATA_EMISSAO >= mskDatEmissaoIniPesq.Text And aviacaoVendas.pDATA_EMISSAO <= mskDatEmissaoFinPesq.Text
            'e.QueryableSource = From aviacaoVendas In db.AviacaoVendas Where aviacaoVendas.pDATA_EMISSAO <= mskDatEmissaoFinPesq.Text
        End If

    End Sub

    Private Sub buttonAplicarFiltro_Click(sender As Object, e As EventArgs) Handles buttonAplicarFiltro.Click
        SqlDataSourceGridPivotAnalise.DataBind()
