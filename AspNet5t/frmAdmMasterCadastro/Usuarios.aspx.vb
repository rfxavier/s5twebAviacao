...
Public Class Usuarios
    ...
    Private Sub InicializaForm()
        ...
        gridRelatoriosMobileUsuario.Selection.UnselectAll()
        gridRelatoriosMobileUsuario.DataBind()
...
#End Region
    ...
    Private Sub PreencheFormComId(ByVal parId As Long)
        ...
        If Not obj Is Nothing Then
            ...
            txtCodigoVendedor.Text = obj.pCodigoVendedor
            txtCodigoVendedorVinculado.Text = obj.pCodigoVendedorVinculado
            txtCodigoSubVendedor.Text = obj.pCodigoSubVendedor
            txtCodigoAprovador.Text = obj.pCodigoAprovador
            ...
            'PREENCHE O gridRelatoriosMobileUsuario, EVENTO DataBound DESTE GRID VERIFICA
            'QUAIS OS TIPOS DEVERÃO SER SELECIONADOS E TEREM OS CHECKBOXES MARCADOS
            gridRelatoriosMobileUsuario.DataBind()
        End If
    End Sub
...
#Region "gridRolesUsuario Tratamento"
    ...
    Protected Sub gridRolesUsuario_DataBinding(sender As Object, e As EventArgs) Handles gridRolesUsuario.DataBinding
        'dim rolesUsuarioUnordered =  S5T.IdRoleCollection.LoadAll.ToList.FindAll(Function(x) x.Name <> "AdmMaster")

        Dim siteMapNodesCollection As SiteMapNodeCollection = ASPxSiteMapDataSource1.Provider.RootNode.ChildNodes

        Dim siteMapNodes = siteMapNodesCollection.Cast(Of SiteMapNode)


        gridRolesUsuario.DataSource = UserRolesOrderedBySiteMap(siteMapNodes)
    End Sub
...
#Region "gridRelatoriosMobileUsuario Tratamento"
    Private Sub gridRelatoriosMobileUsuario_CustomUnboundColumnData(sender As Object, e As ASPxGridViewColumnDataEventArgs) Handles gridRelatoriosMobileUsuario.CustomUnboundColumnData
        If e.Column.FieldName = "pIdComp" Then
            e.Value = DirectCast(sender, ASPxGridView).Selection.IsRowSelected(e.ListSourceRowIndex)
        End If
    End Sub

    Private Class IdRelatoriogridUsuarios
        Public Property pId As Long
        Public Property pTituloRelatorio As String
        Public Property pIdComp As Boolean
    End Class

    Protected Sub gridRelatoriosMobileUsuario_DataBinding(sender As Object, e As EventArgs) Handles gridRelatoriosMobileUsuario.DataBinding
        gridRelatoriosMobileUsuario.DataSource = S5T.AppRelatorioCollection.LoadAll.ToList
    End Sub

    Protected Sub gridRelatoriosMobileUsuario_DataBound(sender As Object, e As EventArgs) Handles gridRelatoriosMobileUsuario.DataBound
        Dim grid As ASPxGridView = TryCast(sender, ASPxGridView)

        If obj IsNot Nothing Then
            If obj.oAppRelatorios IsNot Nothing Then
                Dim listRelatoriosMobileAppRelatorios As New List(Of String)

                For Each objRelatorioMobile In obj.oAppRelatorios
                    listRelatoriosMobileAppRelatorios.Add(objRelatorioMobile.pId.ToString)
                Next

                For i As Integer = 0 To grid.VisibleRowCount - 1
                    If listRelatoriosMobileAppRelatorios.Contains(gridRelatoriosMobileUsuario.GetRowValues(i, "pId").ToString) Then
                        grid.Selection.SelectRow(i)
                    Else
                        grid.Selection.UnselectRow(i)
                    End If
        ...
        Try
            If tipoOperacao() = tpInsAlt.Insercao Then
                ...
                If result.Succeeded Then
                    ...
                    objNewUsuario.pCodigoVendedor = txtCodigoVendedor.Text
                    objNewUsuario.pCodigoVendedorVinculado = txtCodigoVendedorVinculado.Text
                    objNewUsuario.pCodigoSubVendedor = txtCodigoSubVendedor.Text
                    objNewUsuario.pCodigoAprovador = txtCodigoAprovador.Text
                    ...
                    'TRATA ALTERAÇÃO DOS AppRelatorios
                    For j = 0 To gridRelatoriosMobileUsuario.VisibleRowCount - 1
                        If gridRelatoriosMobileUsuario.Selection.IsRowSelected(j) Then
                            'QUER DIZER: INSERÇÃO DE NOVO AppRelatorio
                            Dim objAppRelatorioUsuario = S5T.AppRelatorio.LoadByEntityKey(gridRelatoriosMobileUsuario.GetRowValues(j, "pId").ToString)

                            user.oUsuario.oAppRelatorios.Add(objAppRelatorioUsuario)
                            user.oUsuario.Save()
                        End If
                    Next
                Else
            ...
            ElseIf tipoOperacao() = tpInsAlt.Alteracao Then
                ...
                If objUsuario IsNot Nothing Then
                    ...
                    objUsuario.pCodigoVendedor = txtCodigoVendedor.Text
                    objUsuario.pCodigoVendedorVinculado = txtCodigoVendedorVinculado.Text
                    objUsuario.pCodigoSubVendedor = txtCodigoSubVendedor.Text
                    objUsuario.pCodigoAprovador = txtCodigoAprovador.Text
                    ...
                    'TRATA ALTERAÇÃO DOS AppRelatorios
                    For j = 0 To gridRelatoriosMobileUsuario.VisibleRowCount - 1
                        If gridRelatoriosMobileUsuario.Selection.IsRowSelected(j) And Not gridRelatoriosMobileUsuario.GetRowValues(j, "pIdComp") Then
                            'QUER DIZER: INSERÇÃO DE NOVO AppRelatorio
                            Dim objAppRelatorioUsuario = S5T.AppRelatorio.LoadByEntityKey(gridRelatoriosMobileUsuario.GetRowValues(j, "pId").ToString)

                            user.oUsuario.oAppRelatorios.Add(objAppRelatorioUsuario)
                            user.oUsuario.Save()
                        ElseIf Not gridRelatoriosMobileUsuario.Selection.IsRowSelected(j) And gridRelatoriosMobileUsuario.GetRowValues(j, "pIdComp") Then
                            'QUER DIZER: ELIMINAÇÃO DE AppRelatorio
                            Dim objAppRelatorioUsuario = S5T.AppRelatorio.LoadByEntityKey(gridRelatoriosMobileUsuario.GetRowValues(j, "pId").ToString)

                            user.oUsuario.oAppRelatorios.Remove(objAppRelatorioUsuario)
                            user.oUsuario.Save()
                        End If
                    Next
